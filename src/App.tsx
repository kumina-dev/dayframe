...
export default function App() {
  ...
  const selectedEvent =
    editor?.mode === 'edit'
      ? events.find(
          (event) =>
            event.id === editor.eventId,
        )
      : undefined

  const selectedEventCalendar =
    selectedEvent
      ? calendars.find(
          (calendar) =>
            calendar.id ===
  ...
  const createEvent = async (
    ...
    if (!calendar) {
      throw new Error(
        'The selected calendar does not exist.',
      )
    }
  ...
  const deleteEvent = async (
    ...
    await dayframeDb.events.delete(
      eventId,
    )
  }
  ...
  const updateCalendar = async (
    ...
    const updatedCalendarCount =
      await dayframeDb.calendars.update(
        calendarId,
        {
          ...changes,
          updatedAt:
    ...
    await dayframeDb.transaction(
      ...
      async () => {
        ...
        if (
          ...
          await dayframeDb.settings.update(
            settingsId,
            {
              defaultCalendarId:
        ...
        await dayframeDb.calendars.delete(
          calendarId,
        )
      },
    )
  ...
  const updateProfile = async (
    ...
    const updatedProfileCount =
      await dayframeDb.profile.update(
        localProfileId,
        {
          ...changes,
          updatedAt:
  ...
  const updateIntegration = async (
    ...
    const updatedIntegrationCount =
      await dayframeDb.integrations.update(
        provider,
        {
          ...changes,
          updatedAt:
    ...
    if (
      updatedIntegrationCount === 0
    ) {
      const fallbackIntegration =
        fallbackIntegrations.find(
          (integration) =>
            integration.id ===
            provider,
        )
  ...
  const updateSubscription = async (
    ...
    const updatedSubscriptionCount =
      await dayframeDb.subscription.update(
        localSubscriptionId,
        {
          plan,
          status: 'active',
  ...
  const markAllNotificationsRead =
    async () => {
      await dayframeDb.notifications
        .toCollection()
        .modify({
          isRead: true,
        })
  ...
  const exportCalendarFile = async (
    calendarId: string,
  ) => {
    downloadTextFile(
